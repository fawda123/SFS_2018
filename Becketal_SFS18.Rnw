...
\end{itemize}
\vspace{0.1in}
\centerline{\includegraphics[width=0.9\textwidth]{fig/streams.png}}
\end{frame}
...
p <- ggplot(flds, aes(x = prd, y = obs)) +
  ...
  scale_x_continuous('Predicted Chla') + 
  scale_y_continuous('Observed Chla') +
...
%%%%%%
\begin{frame}{{$\vcenter{\hbox{\includegraphics[width=0.07\paperwidth]{fig/sccwrp_logo.png}}}$\hspace{0.07in}\textbf{4) Identify landscape factors that explain risk}}}
\emtxt{Top five most important watershed factors explaining the relative risk categories} \\~\\
\includegraphics[width = \textwidth]{fig/rskfac.pdf}
\end{frame}

%%%%%%
\begin{frame}{{$\vcenter{\hbox{\includegraphics[width=0.07\paperwidth]{fig/sccwrp_logo.png}}}$\hspace{0.07in}\textbf{What did we learn?}}}
\begin{itemize}
\item<+->National level products can be leveraged for exploratory analysis \\~\\
\item<+-> Chlorophyll could be used as an assessment endpoint of HAB risk \\~\\
\item<+-> Landscape position is a potentially powerful predictor of water quality \\~\\
\item<+-> HAB risks are relatively low statewide, but possibly a byproduct of the model \\~\\
\end{itemize}
\onslide<1->
...
This is a very simplistic model wrought with assumptions, good for exploration but not on a case basis \\~\\
Alternative data acquisition can be explored:
\begin{columns}
\begin{column}{0.5\textwidth}
\centering
...
Photo credits: Meredith Howard, Susan Fricke, Carey Nagoda, Raphael Mazor, Chad Loflen\\~\\}
\end{column}
